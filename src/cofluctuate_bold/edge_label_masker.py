...
from .utils import (create_task_confounders, denoise, standardize,
                    band_pass_dct, ar_whiten, denoise_w_task)


def whiten(x, rho):
    """Whiten a time series array according to AR(1) covariance structure
    Parameters
    ----------
...
class NiftiEdgeAtlas():
    ...
    def __init__(self,
        ...
        self.denoise_task = denoise_task
...
#                assert events.endswith("events.tsv")
        ...
        if self.denoise_task:
            denoise_mat = np.column_stack((task_conf, conf_mat, drifts_mat))
        else:
            denoise_mat = np.column_stack((conf_mat, drifts_mat))
        ...
        if denoise_mat.shape[1] > 0:
            if self.denoise_task:
                ...
                atlas_roi_ts_denoised = denoise_w_task(X=denoise_mat, Y = atlas_roi_ts, T = task_conf)
        else:
...
class NiftiEdgeAtlasAR():
    ...
    def transform(self,
        ...
        # whiten to take into account autocorrelations
        atlas_roi_ts_den_ar = ar_whiten(atlas_roi_ts_denoised)

        # 6-Standardize data
        #  atlas_ts_clean = standardize(atlas_roi_ts_denoised)
        atlas_ts_clean = standardize(atlas_roi_ts_den_ar)

        edge_ts = compute_edge_ts(atlas_ts_clean)
         # Create new image, adding fake affine (old was:run_img.affine)
        edge_img = new_img_like(run_img, edge_ts.astype(np.float32),
                                affine=np.eye(4))
