...
for task_id in ["stroop", "msit"]:
    ...
    for peak_type in ["positive", "negative"]: 
        ...
        for contrast in contrasts:
            ...
            print("computing second-level maps for task %s, peak_type %s and contrast %s" % (task_id, peak_type, contrast))

            # Get first-level effect sizes
            first_level_dir = opj(project_dir, "results/first-level/edge/seed", "task-%s" % task_id, peak_type)
            ...
            # Save this
            output_dir = opj(project_dir, "results/second-level/edge/seed/task-%s" % task_id, peak_type, contrast)
