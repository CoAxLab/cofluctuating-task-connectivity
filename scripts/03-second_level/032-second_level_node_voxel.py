...
for fwhm in [None, 4, 6]:
    print("doing group-level at voxel-level with %s smoothing" % str(fwhm))
    second_level = SecondLevelModel(mask_img = mask_file, smoothing_fwhm=fwhm)
...
#        contrasts = get_contrasts(intercept_only=False)
        ...
        for contrast in contrasts:
            ...
            first_level_dir = opj(project_dir, "results/first-level/node_gsr_voxel", "task-%s" % task_id)
            ...
            # Save this
            output_dir = opj(project_dir, "results/second-level/node_gsr_voxel/smoothing_%s" % str(fwhm), "task-%s" % task_id, contrast)
