...
denoise_opts["hrf_model"] = "glover + derivative + dispersion" # Use this, instead of FIR
...
for task_id in ["stroop", "msit"]:
    ...
    output_dir = opj(project_dir, "results/supplementary/edge_basis_denoise/shen/edge_imgs/task-%s" % task_id)
    Path(output_dir).mkdir(exist_ok=True, parents=True)
    # This is to save intermediate files
    Path(opj(output_dir, "denoised_roi_time_series")).mkdir(exist_ok = True, parents = True)
    Path(opj(output_dir, "denoising_mats")).mkdir(exist_ok = True, parents = True) 
